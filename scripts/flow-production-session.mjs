...
const allowed=new Set(["flow_help","flow_list_accounts","flow_begin_account_connection","flow_complete_account_connection","flow_inspect_account","flow_validate_generation_settings","flow_login_bridge_status","flow_generate_video","flow_job_status","flow_download_job"]);

// This is a client of the dedicated Flow MCP server, not browser automation.
// Production client: paid video generation is authorized in the title directory approval.json; never retry an ambiguous generation.
export async function startFlowProductionSession() {
  ...
  const client=new Client({name:"youtube-flow-production",version:"1.0.0"});
  ...
  return {
    ...
    call:async(name,args={})=>{
      if(!allowed.has(name)||!available.has(name)) throw Error("This production session does not allow "+name);
